...
import { asyncGetOrderListPending } from "#redux/order/action.js";
...
const usePendingOrder = () => {
  ...
  useEffect(() => {
    asyncGetOrderListPending(filterData)
      .then((result) => {
        if (Array.isArray(result?.data?.result)) {
          setPendingOrderList(result?.data?.result);
        }
      })
      .catch((err) => {
  ...
  const filteredData = useMemo(() => {
    return pendingOrderList?.filter((item) =>
      Object.values(item).some((val) =>
        String(val).toLowerCase().includes(search.toLowerCase())
      )
    );
  }, [search, pendingOrderList]);
